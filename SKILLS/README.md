...
2. 起票の一次責任者はマッケンジーとするが、`02_SOLUTIONS/action_learning_program/` のセッションでは、その場に同席する共同進行者（各部門の専門担当者）が「これは他部門にも使えるのでは」と感じた時点でその場で起票候補としてマッケンジーに指摘してよい（現場発の気づきを起点とすることを妨げない）。
...
1. **診断時のスキルカタログ照合を必須化**: `01_DIAGNOSTICS/<部門>` で新規課題が真因特定済みになった時点で、`02_SOLUTIONS` でのソリューション設計に進む前に、マッケンジーは必ず本ページの「スキル一覧・ステータス」表（対象ニーズタグ）と照合し、流用・応用できる既存スキルがないか確認する。ゼロから設計するのは、照合の結果一致するスキルがないと判断した場合に限る。
2. **一致した場合の扱い**: 既存スキルをそのまま複製せず、`SKILL_TEMPLATE.md` の「横展開履歴」に適用部門・適用日・調整点を追記する形で1本化する（RULES.md 1.4、部門間の構造的な違いを踏まえた調整を必須とする）。
3. **申請不要の閲覧性**: `SKILLS/` はリポジトリ内で全部門から閲覧可能とし、部門側からマッケンジーに「こういう課題があるが、近いスキルはあるか」と直接問い合わせることも可能とする（申請書・承認フローを介さない、オンデマンドの情報提供）。
